...
export const generateJWTToken = (user: any) => {
	const JWTtoken = sign({ id: user.id, username: user.username }, JWT_SECRET!, {
...
export const verifyJWTtoken = (
	...
	if (!JWTtoken)
		return res
			.status(401)
			.json({ ok: false, message: "First you have to log in!" });

	try {
		const decode = verify(JWTtoken, JWT_SECRET!);
		console.log(
			`[JWT_] VERIFY: => ${JWTtoken.slice(0, 61).concat("...")} => true `
		);
		next();
	} catch (e) {
...
export const extractJWTdata = (
	...
	const JWTtoken = req.cookies?.token;
	if (!JWTtoken)
		return res.status(401).json({ ok: false, message: "User isn't logged!" });
...
export const destroyJWTdata = (
	...
	const JWTtoken = req.cookies?.token;
	if (!JWTtoken)
		return res.status(401).json({ ok: false, message: "Missing cookie data" });
