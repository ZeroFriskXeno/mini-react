...
export const generateCSRFToken = (req: Request, res: Response) => {
	...
	console.log(
		`[CSRF] CREATE: ${clientID} => ${tokenStore.get(
			clientID
		)} | ORIGIN__: ${originPage}`
	);
};
...
export const verifyCSRFToken = (
	...
	if (!serverToken)
		return res
			.status(403)
			.json({ ok: false, message: "CSRF token expired or invalid" });

	const tokenCheck = !crypto.timingSafeEqual(
	...
	console.log(
		`[CSRF] VERIFY: ${clientID} => ${serverToken} | ENDPOINT: ${originPage} => ${!tokenCheck} `
	);
