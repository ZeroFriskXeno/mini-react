...
export const post_new = async (req: Request, res: Response) => {
	try {
		...
		const { data, error: errorFetch } = await supabase
			.from("posts")
			.select("post_time")
			.eq("user_id", sanitizeInput(id))
			.order("post_time", { ascending: false })
			.limit(1);

		if (errorFetch)
			return res.status(500).json({ ok: false, message: returnError(errorFetch.message) });
		...
		if (errorPost)
			return res.status(500).json({ ok: false, message: returnError(errorPost.message) });

		res.status(201).json({ ok: true, message: "Posted!" });
		console.log("New post created");
	} catch (err) {
		res.status(500).json({ ok: false, message: returnError((err as Error).message) });
	}
};
...
export const post_like = async (req: Request, res: Response) => {
	try {
		...
		const { data: existing_like, error: like_check_error } = await supabase
			.from("likes")
			.select("id")
			.eq("user_id", sanitizeInput(user_id))
			.eq("post_id", sanitizeInput(post_id))
			.single();
